...
for my $dir (@dir) {
	system("ls $workdir/06.kegg.run/02.matrix/$dir/$project\* > $workdir/06.kegg.run/02.matrix/$dir.tmp");
	open (TMP, "<$workdir/06.kegg.run/02.matrix/$dir.tmp") or die $!;
	while (<TMP>) {
		chomp; s/\r//g;
		my $file = $_;
		my $type; my $taxon;
		...
		while (<IN>) {
			...
			if ($line == 1) {
				...
				my $hallj = join "\t", "KEGGnum", @hall;
				print ALL "$hallj\n";
				my $hsamplej = join "\t", "KEGGnum", @hsample;
				print OUT "$hsamplej\n";
				my $hbiorepj = join "\t", "KEGGnum", @hbiorep;
				print BREP "$hbiorepj\n";
				my $htechrepj = join "\t", "KEGGnum", @htechrep;
				print TREP "$htechrepj\n";
				my $hpoolj = join "\t", "KEGGnum", @hpool;
				print POOL "$hpoolj\n";
				my $hqcj = join "\t", "KEGGnum", @hqc;
				print QC "$hqcj\n";
			}else{
				my @data = split /\t/;
