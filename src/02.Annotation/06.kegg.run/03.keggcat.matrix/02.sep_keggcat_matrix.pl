...
for my $dir (@dir) {
	system("ls $workdir/06.kegg.run/03.keggcat.matrix/$dir/$project\* > $workdir/06.kegg.run/03.keggcat.matrix/$dir.tmp");
	open (TMP, "<$workdir/06.kegg.run/03.keggcat.matrix/$dir.tmp") or die $!;
	while (<TMP>) {
		chomp; s/\r//g;
		my $file = $_;
		my $type; my $taxon;
		...
		open (IN, "<$file") or die $!;
		open (BREP, ">$workdir/06.kegg.run/03.keggcat.matrix/$dir/biorep/$project\_diann1.8_$database\_$type\_$taxon\_biorep.txt") or die $!;
		open (TREP, ">$workdir/06.kegg.run/03.keggcat.matrix/$dir/techrep/$project\_diann1.8_$database\_$type\_$taxon\_techrep.txt") or die $!;
		open (POOL, ">$workdir/06.kegg.run/03.keggcat.matrix/$dir/pool/$project\_diann1.8_$database\_$type\_$taxon\_pool.txt") or die $!;
		open (QC, ">$workdir/06.kegg.run/03.keggcat.matrix/$dir/qc/$project\_diann1.8_$database\_$type\_$taxon\_qc.txt") or die $!;
		open (OUT, ">$workdir/06.kegg.run/03.keggcat.matrix/$dir/sample/$project\_diann1.8_$database\_$type\_$taxon\_sample.txt") or die $!;
		open (ALL, ">$workdir/06.kegg.run/03.keggcat.matrix/$dir/all/$project\_diann1.8_$database\_$type\_$taxon\_all.txt") or die $!;
		my $line = 0; my @head;
		...
		while (<IN>) {
			...
			if ($line == 1) {
				...
				my $hallj = join "\t", "KEGGcat", @hall;
				print ALL "$hallj\n";
				my $hsamplej = join "\t", "KEGGcat", @hsample;
				print OUT "$hsamplej\n";
				my $hbiorepj = join "\t", "KEGGcat", @hbiorep;
				print BREP "$hbiorepj\n";
				my $htechrepj = join "\t", "KEGGcat", @htechrep;
				print TREP "$htechrepj\n";
				my $hpoolj = join "\t", "KEGGcat", @hpool;
				print POOL "$hpoolj\n";
				my $hqcj = join "\t", "KEGGcat", @hqc;
				print QC "$hqcj\n";
			}else{
				my @data = split /\t/;
